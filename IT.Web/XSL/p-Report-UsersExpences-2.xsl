			...
			<tr>
				...
				<td>
					<table width="100%" border="0" cellspacing="1" cellpadding="0">
						<tr>
							<td>
								<xsl:for-each select="ShowColumnWorkTimeNorm">
									<xsl:call-template name="std-template-bool">
										<xsl:with-param name="label">Норма рабочего времени</xsl:with-param>
									</xsl:call-template>
								</xsl:for-each>
							</td>
						</tr>
						<tr>
							<td>
								<xsl:for-each select="ShowColumnOverheads">
									<xsl:call-template name="std-template-bool">
										<xsl:with-param name="label">Накладные расходы</xsl:with-param>
									</xsl:call-template>
								</xsl:for-each>
							</td>
						</tr>
						<tr>
							<td>
								<xsl:for-each select="ShowColumnSalaryExpenses">
									<xsl:call-template name="std-template-bool">
										<xsl:with-param name="label">Затраты на з/п</xsl:with-param>
									</xsl:call-template>
								</xsl:for-each>
							</td>
						</tr>
