...
<!--
===============================================================================
	4-я страница параметров отчета "Список инцидентов и затрат проекта"
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:XService="urn:x-client-service" xmlns:q="urn:query-string-access" xmlns:d="urn:object-editor-access" xmlns:w="urn:editor-window-access" xmlns:b="urn:x-page-builder" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template match="FilterReportProjectIncidentsAndExpenses">
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
			<tr>
				<td>
					<xsl:for-each select="Planners">
						<xsl:call-template name="it-template-any-type-objects-tree-selector">
							<xsl:with-param name="prop-names">PlannerOrganizations PlannerDepartments Planners</xsl:with-param>
			...
			<tr>
				<td>
					<xsl:for-each select="IncludePlannerSubDepartments">
						<xsl:call-template name="std-template-bool">
							<xsl:with-param name="label">Включать сотрудников подчиненных подразделений</xsl:with-param>
						</xsl:call-template>
					</xsl:for-each>
				</td>
			</tr>
